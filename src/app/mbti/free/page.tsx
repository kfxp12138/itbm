...
const SCALE_OPTIONS: Array<{ value: MBTIAnswerValue; label: string; size: string }> = [
  { value: 1, label: '非常符合', size: 'h-14 w-14 sm:h-16 sm:w-16' },
  { value: 2, label: '比较符合', size: 'h-12 w-12 sm:h-14 sm:w-14' },
  { value: 3, label: '中立', size: 'h-10 w-10 sm:h-12 sm:w-12' },
  { value: 4, label: '比较符合', size: 'h-12 w-12 sm:h-14 sm:w-14' },
  { value: 5, label: '非常符合', size: 'h-14 w-14 sm:h-16 sm:w-16' },
];
                    ...
                    {SCALE_OPTIONS.map((option) => {
                      ...
                      return (
                        <div key={option.value} className="flex flex-col items-center gap-3 text-center">
                              ...
                              isSelected
                                ? isNeutral
                                  ...
                                  : isLeft
                                    ...
                                    : 'scale-110 border-violet-300 bg-violet-500 text-white shadow-[0_12px_28px_rgba(139,92,246,0.22)]'
                          ...
                          <p className={`text-[10px] font-medium leading-4 sm:text-xs ${isSelected ? 'text-slate-800' : 'text-slate-500'}`}>{option.label}</p>
                        </div>
                      );
                    })}
                  </div>
